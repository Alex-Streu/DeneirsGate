...
var Notiflix = {
    ...
    NotifyContent: {
        ...
        Init: function (userNotifyOpt) {
            // extend options
            newNotifyContentSettings = extendNotiflix(true, notifySettings, userNotifyOpt);
            // use GoogleFonts if "Quicksand"
            notiflixGoogleFont(newNotifyContentSettings.useGoogleFont, newNotifyContentSettings.fontFamily);
        },

        // Merge First Init
        Merge: function (userNotifyExtend) {
            // if initialized already
            if (newNotifyContentSettings) {
                newNotifyContentSettings = extendNotiflix(true, newNotifyContentSettings, userNotifyExtend);
            }
            // initialize first
            else {
                notiflixConsoleError('Notiflix Error', 'You have to initialize the Notify module before call Merge function.');
        ...
        Success: function (message, callback) {
            ...
            var theType = newNotifyContentSettings.success;
            NotiflixNotifyContent(message, callback, theType, 'Success');
        },
        ...
        Failure: function (message, callback) {
            ...
            var theType = newNotifyContentSettings.failure;
            NotiflixNotifyContent(message, callback, theType, 'Failure');
        },
        ...
        Warning: function (message, callback) {
            ...
            var theType = newNotifyContentSettings.warning;
            NotiflixNotifyContent(message, callback, theType, 'Warning');
        },
        ...
        Info: function (message, callback) {
            ...
            var theType = newNotifyContentSettings.info;
            NotiflixNotifyContent(message, callback, theType, 'Info');
        },
    },
    // Notify Content off
...
// Notiflix: Notify Content Single on
var notiflixNotifyContentCount = 0;
var NotiflixNotifyContent = function (message, callback, theType, staticType) {
    if (arguments && arguments.length === 4) {

        // notify counter on
        notiflixNotifyContentCount++;
        // notify counter off
        ...
        // if plainText true = HTML tags not allowed on      
        if (newNotifyContentSettings.plainText) {
            message = notiflixPlaintext(message); // message plain text
        }
        // if plainText true = HTML tags not allowed off

        // if plainText false but the message length more than messageMaxLength = HTML tags error on
        if (!newNotifyContentSettings.plainText && message.length > newNotifyContentSettings.messageMaxLength) {
            Notiflix.NotifyContent.Merge({ closeButton: true, plainText: false, });
            message = '<b>HTML Tags Error:</b> Your content length is more than "messageMaxLength" option.'; // message html error
        }
        // if plainText false but the message length more than messageMaxLength = HTML tags error off

        // message max length substring on
        if (message.length > newNotifyContentSettings.messageMaxLength) {
            message = message.substring(0, newNotifyContentSettings.messageMaxLength) + '...';
        }
        // message max length substring off

        // font awesome icon style on
        if (newNotifyContentSettings.fontAwesomeIconStyle === 'shadow') {
            theType.fontAwesomeIconColor = theType.background;
        }
        // font awesome icon style off

        // if cssAnimaion false -> duration on
        if (!newNotifyContentSettings.cssAnimation) {
            newNotifyContentSettings.cssAnimationDuration = 0;
        }
        // if cssAnimaion false -> duration off

        // notify wrap on
        var docBody = document.body;
        var ntflxNotifyContentWrap = document.createElement('div');
        ntflxNotifyContentWrap.id = notifySettings.wrapID;
        ntflxNotifyContentWrap.style.width = newNotifyContentSettings.width;
        ntflxNotifyContentWrap.style.zIndex = newNotifyContentSettings.zindex;
        ntflxNotifyContentWrap.style.opacity = newNotifyContentSettings.opacity;

        // wrap position on
        if (newNotifyContentSettings.position === 'right-bottom') {
            ntflxNotifyContentWrap.style.right = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.bottom = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.top = 'auto';
            ntflxNotifyContentWrap.style.left = 'auto';
        } else if (newNotifyContentSettings.position === 'left-top') {
            ntflxNotifyContentWrap.style.left = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.top = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.right = 'auto';
            ntflxNotifyContentWrap.style.bottom = 'auto';
        } else if (newNotifyContentSettings.position === 'left-bottom') {
            ntflxNotifyContentWrap.style.left = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.bottom = newNotifyContentSettings.distance;
            ntflxNotifyContentWrap.style.top = 'auto';
            ntflxNotifyContentWrap.style.right = 'auto';
        ...
        // if background overlay true on
        var notifyOverlay;
        if (newNotifyContentSettings.backOverlay) {
            notifyOverlay = document.createElement('div');
            notifyOverlay.id = newNotifyContentSettings.ID + 'Overlay';
            notifyOverlay.style.width = '100%';
            notifyOverlay.style.height = '100%';
            notifyOverlay.style.position = 'fixed';
            notifyOverlay.style.zIndex = newNotifyContentSettings.zindex;
            notifyOverlay.style.left = 0;
            notifyOverlay.style.top = 0;
            notifyOverlay.style.right = 0;
            notifyOverlay.style.bottom = 0;
            notifyOverlay.style.background = newNotifyContentSettings.backOverlayColor;
            notifyOverlay.className = (newNotifyContentSettings.cssAnimation ? 'with-animation' : '');
            notifyOverlay.style.animationDuration = (newNotifyContentSettings.cssAnimation) ? newNotifyContentSettings.cssAnimationDuration + 'ms' : '';
            if (!document.getElementById(notifyOverlay.id)) {
                docBody.appendChild(notifyOverlay);
            }
        }
        ...
        if (!document.getElementById(ntflxNotifyContentWrap.id)) {
            docBody.appendChild(ntflxNotifyContentWrap);
        }
        // notify wrap off

        // notify content on
        var ntflxNotifyContent = document.createElement('div');
        ntflxNotifyContent.id = newNotifyContentSettings.ID + '-' + notiflixNotifyContentCount;
        ntflxNotifyContent.className = newNotifyContentSettings.className + ' ' + theType.childClassName + ' ' + (newNotifyContentSettings.cssAnimation ? 'with-animation' : '') + ' ' + (newNotifyContentSettings.useIcon ? 'with-icon' : '') + ' nx-' + newNotifyContentSettings.cssAnimationStyle + ' ' + (newNotifyContentSettings.closeButton && !callback ? 'with-close-button' : '') + ' ' + (callback && typeof callback === 'function' ? 'with-callback' : '') + ' ' + (newNotifyContentSettings.clickToClose ? 'click-to-close' : '');
        ntflxNotifyContent.style.fontSize = newNotifyContentSettings.fontSize;
        ntflxNotifyContent.style.color = theType.textColor;
        ntflxNotifyContent.style.background = theType.background;
        ntflxNotifyContent.style.borderRadius = newNotifyContentSettings.borderRadius;

        // rtl on
        if (newNotifyContentSettings.rtl) {
            ntflxNotifyContent.setAttribute('dir', 'rtl');
            ntflxNotifyContent.classList.add('rtl-on');
        }
        // rtl off

        // font-family on
        ntflxNotifyContent.style.fontFamily = '"' + newNotifyContentSettings.fontFamily + '"' + ', sans-serif';
        // font-family off

        // use css animation on
        if (newNotifyContentSettings.cssAnimation) {
            ntflxNotifyContent.style.animationDuration = newNotifyContentSettings.cssAnimationDuration + 'ms';
        }
        // use css animation off

        // close button element on
        var closeButtonHTML = '';
        if (newNotifyContentSettings.closeButton && !callback) {
            closeButtonHTML = '<span class="notify-close-button"><svg class="clck2cls" xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="20px" height="20px" version="1.1" style="shape-rendering:geometricPrecision; text-rendering:geometricPrecision; image-rendering:optimizeQuality; fill-rule:evenodd; clip-rule:evenodd"viewBox="0 0 20 20"xmlns:xlink="http://www.w3.org/1999/xlink"><defs><style type="text/css">.click2close{fill:' + theType.notiflixIconColor + '}</style></defs><g><path class="click2close" d="M0.38 2.19l7.8 7.81 -7.8 7.81c-0.51,0.5 -0.51,1.31 -0.01,1.81 0.25,0.25 0.57,0.38 0.91,0.38 0.34,0 0.67,-0.14 0.91,-0.38l7.81 -7.81 7.81 7.81c0.24,0.24 0.57,0.38 0.91,0.38 0.34,0 0.66,-0.14 0.9,-0.38 0.51,-0.5 0.51,-1.31 0,-1.81l-7.81 -7.81 7.81 -7.81c0.51,-0.5 0.51,-1.31 0,-1.82 -0.5,-0.5 -1.31,-0.5 -1.81,0l-7.81 7.81 -7.81 -7.81c-0.5,-0.5 -1.31,-0.5 -1.81,0 -0.51,0.51 -0.51,1.32 0,1.82z"/></g></svg></span>';
        }
        // close buttpon element off

        // use icon on
        if (newNotifyContentSettings.useIcon) {
            // use font awesome
            if (newNotifyContentSettings.useFontAwesome) {
                ntflxNotifyContent.innerHTML = '<i style="color:' + theType.fontAwesomeIconColor + '; font-size:' + newNotifyContentSettings.fontAwesomeIconSize + ';" class="nmi wfa ' + theType.fontAwesomeClassName + ' ' + (newNotifyContentSettings.fontAwesomeIconStyle === 'shadow' ? 'shadow' : 'basic') + '"></i><span class="the-message with-icon">' + message + '</span>' + (newNotifyContentSettings.closeButton ? closeButtonHTML : '');
            }
            // use notiflix icon
            else {
                var svgIcon;
                ...
                ntflxNotifyContent.innerHTML = svgIcon + '<span class="the-message with-icon">' + message + '</span>' + (newNotifyContentSettings.closeButton ? closeButtonHTML : '');
            }
        }
        // without icon
        else {
            ntflxNotifyContent.innerHTML = '<span class="the-message">' + message + '</span>' + (newNotifyContentSettings.closeButton ? closeButtonHTML : '');
        }
        // use icon off
        // notify content off

        // notify append or prepend on
        if (newNotifyContentSettings.position === 'left-bottom' || newNotifyContentSettings.position === 'right-bottom') { // the new one will be first
            var notifyWrap = document.getElementById(ntflxNotifyContentWrap.id);
            notifyWrap.insertBefore(ntflxNotifyContent, notifyWrap.firstChild);
        } else {
            document.getElementById(ntflxNotifyContentWrap.id).appendChild(ntflxNotifyContent);
        }

        if (newNotifyContentSettings.useIcon) { // if useIcon, dynamically vertical align the contents
            var messageIcon = document.getElementById(ntflxNotifyContent.id).querySelectorAll('.nmi')[0];
            var messageIconH = 40;
            // if font awesome
            if (newNotifyContentSettings.useFontAwesome) {
                messageIconH = Math.round(parseInt(messageIcon.offsetHeight));
            ...
            var messageText = document.getElementById(ntflxNotifyContent.id).querySelectorAll('span')[0];
            var messageTextH = Math.round(messageText.offsetHeight);
            if (messageTextH <= messageIconH) {
                messageText.style.paddingTop = (messageIconH - messageTextH) / 2 + 'px';
                messageText.style.paddingBottom = (messageIconH - messageTextH) / 2 + 'px';
        ...
        // remove by timeout or click on
        if (document.getElementById(ntflxNotifyContent.id)) {
            // set elements on
            var removeDiv = document.getElementById(ntflxNotifyContent.id);
            var removeWrap = document.getElementById(ntflxNotifyContentWrap.id);
            var removeOverlay;
            if (newNotifyContentSettings.backOverlay) {
                removeOverlay = document.getElementById(notifyOverlay.id);
            }
            ...
            // hide notify elm and hide overlay on
            var hideNotifyContentElementsAndOverlay = function () {
                removeDiv.classList.add('remove');
                if (newNotifyContentSettings.backOverlay && removeWrap.childElementCount <= 0) {
                    removeOverlay.classList.add('remove');
                }
                clearTimeout(timeoutHide);
            };
            ...
            // remove notify elm and wrapper on
            var removeNotifyContentElmentsAndWrapper = function () {
                var notifyExist = document.getElementById(ntflxNotifyContent.id);
                if (notifyExist && removeDiv.parentNode !== null) {
                    removeDiv.parentNode.removeChild(removeDiv);
                }
                if (removeWrap.childElementCount <= 0 && removeWrap.parentNode !== null) { // if childs count === 0 remove wrap
                    removeWrap.parentNode.removeChild(removeWrap);
                    if (newNotifyContentSettings.backOverlay && removeOverlay.parentNode !== null) {
                        removeOverlay.parentNode.removeChild(removeOverlay);
                    }
                }
                clearTimeout(timeoutRemove);
            ...
            // if close button and callback undefined on            
            if (newNotifyContentSettings.closeButton && !callback) {
                var closeButtonElm = document.getElementById(ntflxNotifyContent.id).querySelectorAll('span.notify-close-button')[0];
                closeButtonElm.addEventListener('click', function () {
                    hideNotifyContentElementsAndOverlay();
                    var clickToCloseTimeout = setTimeout(function () {
                        removeNotifyContentElmentsAndWrapper();
                        clearTimeout(clickToCloseTimeout);
                    }, newNotifyContentSettings.cssAnimationDuration);
                });
            }
            // if close button and callback undefined off

            // if callback or click to close on
            if ((callback && typeof callback === 'function') || newNotifyContentSettings.clickToClose) {
                removeDiv.addEventListener('click', function (e) {
                    if (callback && typeof callback === 'function') {
                        callback();
                    }
            ...
            if (!newNotifyContentSettings.closeButton && !callback) {
                timeoutHide = setTimeout(function () {
                    hideNotifyContentElementsAndOverlay();
                }, newNotifyContentSettings.timeout);
                timeoutRemove = setTimeout(function () {
                    removeNotifyContentElmentsAndWrapper();
                }, newNotifyContentSettings.timeout + newNotifyContentSettings.cssAnimationDuration);
            }
            // else auto remove off
        }
        // remove by timeout or click off
        ...
        // notify - show only the last one on
        if (newNotifyContentSettings.showOnlyTheLastOne && notiflixNotifyContentCount > 0) {
            var allNotifyContentElmNotTheLastOne = document.querySelectorAll('[id^=' + newNotifyContentSettings.ID + '-]:not([id=' + newNotifyContentSettings.ID + '-' + notiflixNotifyContentCount + '])');
            for (var i = 0; i < allNotifyContentElmNotTheLastOne.length; i++) {
                var eachNotifyContentElmNotLastOne = allNotifyContentElmNotTheLastOne[i];
                if (eachNotifyContentElmNotLastOne.parentNode !== null) {
                    eachNotifyContentElmNotLastOne.parentNode.removeChild(eachNotifyContentElmNotLastOne);
                }
            }
        }
        // notify - show only the last one off
...
// Notiflix: Notify Content Single off
