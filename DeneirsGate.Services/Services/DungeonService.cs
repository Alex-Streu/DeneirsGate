...
namespace DeneirsGate.Services
{
    public class DungeonService : DeneirsService
    {
        ...
        public DungeonService(DataEntities _db)
        {
            db = _db;
        }
        ...
        public List<Guid> DeleteDungeon(Guid userId, Guid campaignId, Guid dungeonId)
        {
            ...
            var encounterIds = db.DungeonTileEncounters.Where(x => tileIds.Contains(x.TileKey)).Select(x => x.EncounterKey).ToList() ?? new List<Guid>();
        ...
        {
            var list = db.TrapNatures.OrderBy(x => x.Name).Select(x => new TrapNatureViewModel
            {
                Name = x.Name,
                NatureKey = x.NatureKey
            }).ToList();
        ...
        {
            var list = db.TrapTypes.OrderByDescending(x => x.Name == "Setback")
